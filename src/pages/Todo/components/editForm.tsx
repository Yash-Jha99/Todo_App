...
import { TableListItem } from './TodoTable';
...
type Props = {
  ...
  handleEditFormOpen: (value: React.SetStateAction<boolean>) => void;
  handleEdit: (value: API.TodoListItem) => Promise<boolean>;
  todoData: TableListItem | undefined;
  actionRef: any;
};

const EditForm = ({ handleEditFormOpen, editFormOpen, handleEdit, actionRef, todoData }: Props) => {
  ...
  return (
    <ModalForm<Todo>
      title="Edit Todo"
      form={form}
      onOpenChange={handleEditFormOpen}
      open={editFormOpen}
      autoFocusFirstInput
      modalProps={{
        destroyOnClose: true,
      ...
      onFinish={async (value) => {
        ...
        const success = await handleEdit(reqBody as API.TodoListItem);
        if (success) {
          handleEditFormOpen(false);
          setTags([]);
    ...
    >
      <ProForm.Group>
        ...
        <ProFormSelect
          ...
          name="status"
          label="Status"
          initialValue={todoData?.status}
          required
        />
        <ProFormDatePicker initialValue={todoData?.dueDate} name="dueDate" label="Due Date" />
      </ProForm.Group>
      <ProForm.Group>
        <ProFormTextArea
